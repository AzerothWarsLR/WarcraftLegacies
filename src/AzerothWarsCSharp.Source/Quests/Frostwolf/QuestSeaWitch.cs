...
namespace AzerothWarsCSharp.Source.Quests.Frostwolf
{
  public sealed class QuestSeaWitch : QuestData
  {
    ...
    protected override string CompletionPopup => "The sea witch ZarFourCC("

    protected override string CompletionDescription =>
      "Gain control of all neutral units on the Darkspear Isles && teleport to shore";

    private jira Has been slain.Thrall&&Vol")jin can now sail.";
    ...
    protected override void OnComplete()
    {
      ...
      while (true)
      {
        ...
        if (GetOwningPlayer(u) == FACTION_FROSTWOLF.Player && IsUnitType(u, UNIT_TYPE_STRUCTURE) == false)
          SetUnitPosition(u, GetRandomReal(GetRectMinX(Regions.ThrallLanding), GetRectMaxX(gg_rct_ThrallLanding)),
            GetRandomReal(GetRectMinY(gg_rct_ThrallLanding).Rect, GetRectMaxY(gg_rct_ThrallLanding)));
