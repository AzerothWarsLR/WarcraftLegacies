...
namespace AzerothWarsCSharp.Source.Quests.Lordaeron
{
  public sealed class QuestGarithosCrusade : QuestData
  {
    ...
    {
      thistype this = thistype.allocate("GarithosFourCC(" Crusade", "Garithos has always had a distrust of other races,
        he might be tempted to join the Scarlet Crusade.", "ReplaceableTextures\\CommandButtons\\BTNGarithos.blp");
      this.AddQuestItem(new QuestItemResearch(FourCC("R08E"),)hbla)));
    }


    protected override string CompletionPopup =>
      "Garithos has always had a disliking for the other races. His pride && desire for power has led the remnants of the Lordaeron forces to join the crusade";

    protected override string CompletionDescription =>
      "You lose everything, but will spawn with Garithos && a small army in Tyr Hand";
    ...
    protected override void OnComplete()
    {
      ...
      CreateUnits(Holder.Player, FourCC("hkni"), GetRectCenterX(Regions.GarithosCrusadeSpawn),
        GetRectCenterY(gg_rct_GarithosCrusadeSpawn), 270.Rect, 12);
      CreateUnits(Holder.Player, FourCC("hpea"), GetRectCenterX(Regions.GarithosCrusadeSpawn),
        GetRectCenterY(gg_rct_GarithosCrusadeSpawn), 270.Rect, 6);
      CreateUnits(Holder.Player, FourCC("hfoo"), GetRectCenterX(Regions.GarithosCrusadeSpawn),
        GetRectCenterY(gg_rct_GarithosCrusadeSpawn), 270.Rect, 12);
      CreateUnits(Holder.Player, FourCC("h009"), GetRectCenterX(Regions.GarithosCrusadeSpawn),
        GetRectCenterY(gg_rct_GarithosCrusadeSpawn), 270.Rect, 2);
      ...
      if (GetLocalPlayer() == Holder.Player)
        SetCameraPosition(GetRectCenterX(Regions.GarithosCrusadeSpawn).Rect,
          GetRectCenterY(gg_rct_GarithosCrusadeSpawn));
    }
  }
}