...
public static class GilneasObjectInfo
{
  ...
  public static IEnumerable<ObjectInfo> GetAllObjectLimits()
  {
    ...
    yield return new(UNIT_H01T_INN_LORDAERON_OTHER, Unlimited, UnitCategory.Farm); //Farm 2
    yield return new(UNIT_H05P_DALARAN_BUILDING_DALARAN_OTHER_1, Unlimited, UnitCategory.Farm); //farm in Dalaran
    yield return new(UNIT_H05Q_DALARAN_BUILDING_DALARAN_OTHER_2, Unlimited, UnitCategory.Farm); //farm in Dalaran
    yield return new(UNIT_H05O_DALARAN_BUILDING_DALARAN_OTHER_3, Unlimited, UnitCategory.Farm); //farm in Dalaran
    ...
    yield return new(UNIT_H03A_GUARD_TOWER_GILNEAS_TOWER, Unlimited, UnitCategory.Tower); //Guard Tower
    yield return new(UNIT_H03B_CANNON_TOWER_GILNEAS_TOWER, Unlimited, UnitCategory.Tower); //Cannon Tower
    yield return new(UNIT_H052_IMPROVED_GUARD_TOWER_GILNEAS_TOWER, Unlimited, UnitCategory.Tower); //Improved Guard Tower
    yield return new(UNIT_H04N_IMPROVED_CANNON_TOWER_GILNEAS_TOWER, Unlimited, UnitCategory.Tower); //Improved Cannon Tower
    yield return new(UNIT_N03G_VIOLET_TOWER_DALARAN, Unlimited, UnitCategory.Tower); //Violet Towers

    yield return new(UNIT_HPEA_PEASANT_LORDAERON_STORMWIND_WORKER, Unlimited, UnitCategory.Worker); //Worker
    yield return new(UNIT_H04M_CLERIC_GILNEAS, Unlimited, UnitCategory.Support); //Cleric
    yield return new(UNIT_N06K_DRUID_OF_THE_SCYTHE_GILNEAS, Unlimited, UnitCategory.Support); //Druid of the Scythe
    yield return new(UNIT_H04X_HARVEST_WITCH_GILNEAS, 6, UnitCategory.Support); //HarvestWitch
    yield return new(UNIT_O06P_WORGEN_SHAMAN_GILNEAS, 6, UnitCategory.Support); //Worgen Shaman

    yield return new(UNIT_H04E_PROTECTOR_GILNEAS, Unlimited, UnitCategory.Fighter); //Protector
    yield return new(UNIT_N06L_ARMORED_WOLF_GILNEAS, Unlimited, UnitCategory.Fighter); //Armored Wolf
    yield return new(UNIT_O01V_GREYGUARD_GILNEAS, 6, UnitCategory.Elite); //Greyguard
    yield return new(UNIT_O02J_WORGEN_GILNEAS, 12, UnitCategory.Fighter); //Worgen

    yield return new(UNIT_H03L_SHOTGUNNER_GILNEAS, Unlimited, UnitCategory.Marksman); //Shotgunner
    yield return new(UNIT_O04U_CYCLONE_CANNON_GILNEAS, 6, UnitCategory.SiegeWorkshop); //Cyclone Cannon
