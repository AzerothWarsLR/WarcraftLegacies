...
public sealed class AdvancedMapBuilder(AdvancedMapBuilderOptions options)
{
  ...
  public void SaveMapFile(Map map, IEnumerable<DirectoryEnumerationOptions> additionalFileDirectories)
  {
    ...
    mapBuilder.Build(options.OutputPath, archiveCreateOptions);
    Console.WriteLine($"Published map archive to {Path.GetFullPath(options.OutputPath)}.");
  }
  ...
  public void SaveMapDirectory(Map map, IEnumerable<PathData> additionalFiles)
  {
    ...
    if (options.ShouldBackup && Directory.Exists(options.OutputPath))
    {
      if (options.ShouldBackup)
      {
        BackupFiles(options.BackupPath, options.OutputPath);
      }
