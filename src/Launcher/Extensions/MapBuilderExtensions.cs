...
namespace Launcher.Extensions
{
  public static class MapBuilderExtensions
  {
    ...
    /// </summary>
    public static void BuildFolder(this MapBuilder mapBuilder, string rootFolderPath)
    {
      ...
      if (map.DestructableObjectData != null)Write(rootFolderPath, DestructableObjectData.MapFileName, map.DestructableObjectData);
      ...
      if (map.AbilitySkinObjectData != null) Write(rootFolderPath, AbilityObjectData.MapSkinFileName, map.AbilitySkinObjectData);
      if (map.BuffSkinObjectData != null) Write(rootFolderPath, BuffObjectData.MapSkinFileName, map.BuffSkinObjectData);
      if (map.DestructableSkinObjectData != null) Write(rootFolderPath, DestructableObjectData.MapSkinFileName, map.DestructableSkinObjectData);
      if (map.DoodadSkinObjectData != null) Write(rootFolderPath, DoodadObjectData.MapSkinFileName, map.DoodadSkinObjectData);
      ...
      if (map.UpgradeSkinObjectData != null) Write(rootFolderPath, UpgradeObjectData.MapSkinFileName, map.UpgradeSkinObjectData);
    ...
    private static void Write(string rootFolderPath, string subFolderPath, DestructableObjectData destructableObjectData)
    {
      var path = Path.Combine(rootFolderPath, subFolderPath);
      using var stream = File.Open(path, FileMode.Create);
      using var writer = new BinaryWriter(stream, Encoding.UTF8, false);
