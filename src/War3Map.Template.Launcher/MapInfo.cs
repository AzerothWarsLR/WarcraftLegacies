            ...
            int initialCliffLevel = 2)
        {
            ...
            for (nint y = 0; y <= mapHeight; y++)
            {
                for (nint x = 0; x <= mapWidth; x++)
                {
                    ...
                    tile.CliffLevel = initialCliffLevel;
                    tile.CliffTexture = 15;
                    tile.CliffVariation = 1;

                    tile.Height = 0;
                    tile.IsBlighted = false;
                    tile.IsBoundary = false;
                    tile.IsEdgeTile = x != mapWidth && y != mapHeight && (x < edgeLeft || x >= edgeRight || y < edgeBottom || y >= edgeTop);
                    tile.IsRamp = false;
                    tile.IsWater = false;
                    tile.Texture = initialTileTexture;
                    tile.Variation = 0;
                    tile.WaterHeight = 0;
