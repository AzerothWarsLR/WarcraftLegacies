...
namespace WarcraftLegacies.Source.Factions.Skywall.Powers;
...
public sealed class Windforging : Power
{
  ...
  public Windforging(int unitTypeId, float chance, Point returnPoint, string returnPointName, Rectangle noReturnRect)
  {
    ...
    Description = Loc.Format(
      "Your non-Resistant units have a {chance}% chance to be transfigured into a {unit} in {location} upon death.",
      ("{chance}", (chance * 100).ToString()),
      ("{unit}", GetObjectName(_unitTypeId)),
      ("{location}", returnPointName));
  }
