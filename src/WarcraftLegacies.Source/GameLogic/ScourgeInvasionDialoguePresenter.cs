...
namespace WarcraftLegacies.Source.GameLogic
{
  public sealed class ScourgeInvasionDialoguePresenter : ChoiceDialoguePresenter
  {
    ...
    private void PickLocation(object? sender, ChoiceDialoguePresenterEventArgs args)
    {
      ...
      foreach (var unit in CreateGroup().EnumUnitsInRect(Rectangle.WorldBounds).EmptyToList().Where(x => x.OwningPlayer() == args.Player))
      {
        if (IsUnitType(unit, UNIT_TYPE_STRUCTURE) || IsUnitType(unit, UNIT_TYPE_ANCIENT)) continue;
