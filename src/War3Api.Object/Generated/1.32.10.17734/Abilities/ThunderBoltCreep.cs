...
namespace War3Api.Object.Abilities
{
    public sealed class ThunderBoltCreep : Ability
    {
        ...
        public ThunderBoltCreep(ObjectDatabaseBase db): base(1651786561, db)
        {
            _dataDamage = new Lazy<ObjectProperty<float>>(() => new ObjectProperty<float>(GetDataDamage, SetDataDamage));
            _isDataDamageModified = new Lazy<ReadOnlyObjectProperty<bool>>(() => new ReadOnlyObjectProperty<bool>(GetIsDataDamageModified));
        }

        public ThunderBoltCreep(int newId, ObjectDatabaseBase db): base(1651786561, newId, db)
        {
            _dataDamage = new Lazy<ObjectProperty<float>>(() => new ObjectProperty<float>(GetDataDamage, SetDataDamage));
            _isDataDamageModified = new Lazy<ReadOnlyObjectProperty<bool>>(() => new ReadOnlyObjectProperty<bool>(GetIsDataDamageModified));
        }

        public ThunderBoltCreep(string newRawcode, ObjectDatabaseBase db): base(1651786561, newRawcode, db)
        {
            _dataDamage = new Lazy<ObjectProperty<float>>(() => new ObjectProperty<float>(GetDataDamage, SetDataDamage));
            _isDataDamageModified = new Lazy<ReadOnlyObjectProperty<bool>>(() => new ReadOnlyObjectProperty<bool>(GetIsDataDamageModified));
        }
        ...
        {
            return _modifications.GetModification(828535875, level).ValueAsFloat;
        }
