...
library SpawnTentacle3 initializer OnInit requires T32

    globals
        private constant integer UNIT3_ID = 'U02C'
        private constant integer TENTACLE_COUNT_BASE = 5
        private constant integer TENTACLE3_ID = 'n0B7'
        private constant real RADIUS_OFFSET = 500.  //How far away from the caster to position the Tentacles
    endglobals    

    struct tentacleAppendageSet3
        ...
        method spawnTentacle takes real x, real y, integer index returns nothing
            local unit tempUnit = CreateUnit(GetOwningPlayer(this.caster), TENTACLE3_ID, x, y, 0)
    ...
    private function Built takes nothing returns nothing
        local tentacleAppendageSet3 temptentacleAppendageSet3 = 0
        local unit triggerUnit = null
        local integer triggerUnitHandleId = 0
            set triggerUnit = gg_unit_U02C_2829
            set triggerUnitHandleId = UNIT3_ID
            set temptentacleAppendageSet3 = tentacleAppendageSet3.tentacleSetsByCasterHandleId[triggerUnitHandleId]
            if temptentacleAppendageSet3 == 0 then
                set temptentacleAppendageSet3 = tentacleAppendageSet3.create(triggerUnit)
                set tentacleAppendageSet3.tentacleSetsByCasterHandleId[triggerUnitHandleId] = temptentacleAppendageSet3
            endif
